...
// Generate temperature overlay data - dense grid for smooth gradient
const generateTemperatureData = () => {
  const data = [];
  for (let lat = -80; lat <= 80; lat += 5) {
    for (let lng = -180; lng <= 180; lng += 5) {
      // Simulate temperature based on latitude (warmer near equator)
      const temp = 30 - Math.abs(lat) * 0.5 + (Math.random() - 0.5) * 8;
      data.push({ lat, lng, temp });
    }
  }
  return data;
};
        ...
        {/* Globe container */}
          <div className="flex-1 h-[calc(100vh-80px)] relative bg-slate-950">
            <Globe
              ...
              pointLng="lng"
              pointColor={d => {
                if (!d || d.intensity === undefined) return 'rgba(100, 100, 100, 0.5)';
                if (d.intensity > 0.8) return 'rgba(239, 68, 68, 0.7)';
                if (d.intensity > 0.6) return 'rgba(249, 115, 22, 0.7)';
                if (d.intensity > 0.4) return 'rgba(234, 179, 8, 0.7)';
                return 'rgba(34, 197, 94, 0.7)';
              }}
              pointAltitude={0.02}
              pointRadius={d => d && d.intensity !== undefined ? 0.4 + (d.intensity * 0.6) : 0.3}
              ...
              heatmapsData={activeLayers.temperature ? [temperatureData] : []}
              heatmapPointLat={d => d.lat}
              heatmapPointLng={d => d.lng}
              heatmapPointWeight={d => d.temp}
              heatmapBandwidth={4}
              heatmapColorFn={(t) => {
                if (t < 0.2) return 'rgba(37, 99, 235, 0.6)';
                if (t < 0.4) return 'rgba(59, 130, 246, 0.7)';
                if (t < 0.6) return 'rgba(234, 179, 8, 0.7)';
                if (t < 0.8) return 'rgba(249, 115, 22, 0.8)';
